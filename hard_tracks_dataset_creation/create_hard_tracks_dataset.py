...
def copy(src, dst):
    ...
    def ignore_non_images(dir, files):
        ...
        return [f for f in files if not (
                os.path.isdir(os.path.join(dir, f)) or f.lower().endswith(('.jpg', '.png', '.jpeg'))
        )]
...
def process_hard_tracks_tao(hard_tracks, annotations_path, original_dataset_path, hard_tracks_dataset_path):
    ...
    # Copy videos
    all_videos = []
    for dataset in os.listdir(os.path.join(original_dataset_path, "frames", "val")):
        for video in os.listdir(os.path.join(original_dataset_path, "frames", "val", dataset)):
            all_videos.append(f"val/{dataset}/{video}")
    ...
    for video_folder in tqdm(video_folders_to_copy):
        ...
        video_path = os.path.join(original_dataset_path, "frames", "val", video_folder.split("/")[1], video_folder.split("/")[2])
...
def process_hard_tracks_bdd(hard_tracks, annotations_path, original_dataset_path, hard_tracks_dataset_path):
    ...
    # Copy videos
    video_folders_to_copy = [v for v in os.listdir(os.path.join(original_dataset_path, "bdd100k", "images", "track", "val")) if v in hard_tracks_video_names]
    for video_folder in tqdm(video_folders_to_copy):
        # copy whole folder to destination
        video_path = os.path.join(original_dataset_path, "bdd100k", "images", "track", "val", video_folder)
...
def process_hard_tracks_got(hard_tracks, annotations_path, original_dataset_path, hard_tracks_dataset_path):
    ...
    video_folders_to_copy = [v for v in os.listdir(os.path.join(original_dataset_path, "val")) if v in hard_tracks_video_names]
    for video_folder in tqdm(video_folders_to_copy):

        # copy whole folder to destination
        video_path = os.path.join(original_dataset_path, "val", video_folder)
        video_path_dst = os.path.join(hard_tracks_dataset_path, "data", target_folder, video_folder)
        copy(video_path, video_path_dst)
...
def process_hard_tracks_sportsmot(hard_tracks, annotations_path, original_dataset_path, hard_tracks_dataset_path):
    ...
    video_folders_to_copy = [v for v in os.listdir(os.path.join(original_dataset_path, "dataset", "val")) if v in hard_tracks_video_names]
    for video_folder in tqdm(video_folders_to_copy):
        # copy whole folder to destination
        video_path = os.path.join(original_dataset_path, "dataset", "val", video_folder, "img1")
        video_path_dst = os.path.join(hard_tracks_dataset_path, "data", target_folder, video_folder)
        copy(video_path, video_path_dst)
...
def process_hard_tracks_dancetrack(hard_tracks, annotations_path, original_dataset_path, hard_tracks_dataset_path):
    ...
    for video_folder in tqdm(video_folders_to_copy):
        # copy whole folder to destination
        video_path = os.path.join(original_dataset_path, "val", video_folder, "img1")
        video_path_dst = os.path.join(hard_tracks_dataset_path, "data", target_folder, video_folder)
        copy(video_path, video_path_dst)
...
def process_hard_tracks_bft(hard_tracks, annotations_path, original_dataset_path, hard_tracks_dataset_path):
    ...
    video_folders_to_copy = [v for v in os.listdir(os.path.join(original_dataset_path, "frames", "val")) if v in hard_tracks_video_names]
    for video_folder in tqdm(video_folders_to_copy):
        # copy whole folder to destination
        video_path = os.path.join(original_dataset_path, "frames", "val", video_folder)
        video_path_dst = os.path.join(hard_tracks_dataset_path, "data", target_folder, video_folder)
        copy(video_path, video_path_dst)
...
def process_hard_tracks_uvo(hard_tracks, annotations_path):
    pass
...
def main():
    ...
    folder_path = "hard_tracks_dataset3"
    ...
    statistics_tao = pd.read_csv(os.path.join(statistics_path, "statistics_TAO.csv"))
    statistics_bdd = pd.read_csv(os.path.join(statistics_path, "statistics_BDD.csv"))
    statistics_got = pd.read_csv(os.path.join(statistics_path, "statistics_GOT.csv"))
    statistics_lasot = pd.read_csv(os.path.join(statistics_path, "statistics_LASOT.csv"))
    statistics_ovtb = pd.read_csv(os.path.join(statistics_path, "statistics_OVTB.csv"))
    statistics_animaltrack = pd.read_csv(os.path.join(statistics_path, "statistics_ANIMALTRACK.csv"))
    statistics_sportsmot = pd.read_csv(os.path.join(statistics_path, "statistics_SPORTSMOT.csv"))
    statistics_dancetrack = pd.read_csv(os.path.join(statistics_path, "statistics_DANCETRACK.csv"))
    statistics_bft = pd.read_csv(os.path.join(statistics_path, "statistics_BFT.csv"))

    # Filter hard tracks
    hard_tracks_tao = statistics_tao[((statistics_tao["number_of_occlusions"] >= 5) | (statistics_tao["max_occlusion_length"] >= 15))]
    hard_tracks_bdd = statistics_bdd[(statistics_bdd["number_of_occlusions"] >= 5) | (statistics_bdd["max_occlusion_length"] >= 50)]
    hard_tracks_got = statistics_got[(statistics_got["number_of_occlusions"] >= 1)]
    hard_tracks_lasot = statistics_lasot[(statistics_lasot["number_of_occlusions"] >= 20) & (statistics_lasot["track_length"] <= 4000)]
    hard_tracks_ovtb = statistics_ovtb[((statistics_ovtb["number_of_occlusions"] >= 3) | (statistics_ovtb["max_occlusion_length"] >= 20))]
    hard_tracks_animaltrack = statistics_animaltrack[(statistics_animaltrack["max_occlusion_length"] >= 50)]
    hard_tracks_sportsmot = statistics_sportsmot[(statistics_sportsmot["number_of_occlusions"] >= 10) | (statistics_sportsmot["max_occlusion_length"] >= 50)]
    hard_tracks_dancetrack = statistics_dancetrack[(statistics_dancetrack["number_of_occlusions"] >= 10) | (statistics_dancetrack["max_occlusion_length"] >= 50)]
    hard_tracks_bft = statistics_bft[(statistics_bft["number_of_occlusions"] >= 2) | (statistics_bft["max_occlusion_length"] >= 40)]

    # Process hard tracks
    tasks = [
        (process_hard_tracks_tao, (hard_tracks_tao, "../data/tao/annotations/tao_val_lvis_v1_classes.json", "../data/tao/",os.path.join(dataset_path, folder_path))),
        (process_hard_tracks_bdd, (hard_tracks_bdd, "../data/bdd/annotations/bdd_coco_val.json", "../data/bdd/",os.path.join(dataset_path, folder_path))),
        (process_hard_tracks_got, (hard_tracks_got, "../data/got10k/annotations/got_coco_val.json", "../data/got10k/",os.path.join(dataset_path, folder_path))),
        (process_hard_tracks_lasot, (hard_tracks_lasot, "../data/lasot/annotations/lasot_coco_val.json", "../data/lasot/",os.path.join(dataset_path, folder_path))),
        (process_hard_tracks_ovtb, (hard_tracks_ovtb, "../data/ovtb/ovtb_coco_val.json", "../data/ovtb/",os.path.join(dataset_path, folder_path))),
        (process_hard_tracks_animaltrack, (hard_tracks_animaltrack, "../data/animaltrack/annotations/animaltrack_coco_val.json", "../data/animaltrack/",os.path.join(dataset_path, folder_path))),
        (process_hard_tracks_sportsmot, (hard_tracks_sportsmot, "../data/metadata/sportsmot_coco_val.json", "../data/sportsmot/",os.path.join(dataset_path, folder_path))),
        (process_hard_tracks_dancetrack, (hard_tracks_dancetrack, "../data/dancetrack/annotations/dancetrack_coco_val.json", "../data/dancetrack/",os.path.join(dataset_path, folder_path))),
        (process_hard_tracks_bft, (hard_tracks_bft, "../data/bft/annotations/bft_coco_val.json", "../data/bft/",os.path.join(dataset_path, folder_path))),
    ]

    results = []

    def run_processing(task):
        func, args = task
        return func(*args)

    # Run them in parallel
    with ThreadPoolExecutor() as executor:
        results = list(executor.map(run_processing, tasks))
    ...
    #annotations_bdd = process_hard_tracks_bdd(hard_tracks_bdd, "../data/bdd/annotations/bdd_coco_val.json", "../data/bdd/", os.path.join(dataset_path, folder_path))
    #annotations_got = process_hard_tracks_got(hard_tracks_got, "../data/got10k/annotations/got_coco_val.json", "../data/got10k/", os.path.join(dataset_path, folder_path))
    #annotations_lasot = process_hard_tracks_lasot(hard_tracks_lasot, "../data/lasot/annotations/lasot_coco_val.json", "../data/lasot/", os.path.join(dataset_path, folder_path))
    ...
    #annotations_animaltrack = process_hard_tracks_animaltrack(hard_tracks_animaltrack, "../data/animaltrack/annotations/animaltrack_coco_val.json", "../data/animaltrack/", os.path.join(dataset_path, folder_path))
    #annotations_sportsmot = process_hard_tracks_sportsmot(hard_tracks_sportsmot, "../data/metadata/sportsmot_coco_val.json", "../data/sportsmot/", os.path.join(dataset_path, folder_path))
    #annotations_dancetrack = process_hard_tracks_dancetrack(hard_tracks_dancetrack, "../data/dancetrack/annotations/dancetrack_coco_val.json", "../data/dancetrack/", os.path.join(dataset_path, folder_path))
    #annotations_bft = process_hard_tracks_bft(hard_tracks_bft, "../data/bft/annotations/bft_coco_val.json", "../data/bft/", os.path.join(dataset_path, folder_path))
