...
class BDDTETAMetric(BaseVideoMetric):
    ...
    def compute_metrics(self, results: list = None) -> dict:
        ...
        # Quick fix to make bdd runnable for other groundtruths
        video_names_map = {r[0].videoName: True for r in results}
        print("video_names_map", video_names_map.keys())
        gts = [g for g in gts if g[0].videoName in video_names_map]
        print("results", len(results), "gts", len(gts))
