...
class HardTracksTETAMetric(BaseVideoMetric):
    ...
    default_prefix: Optional[str] = "hard_tracks_teta_metric"
    ...
    def __init__(
        ...
        dataset_type: str = "HardTracksDataset",
        ...
        assert dataset_type in ["HardTracksDataset"]
        if dataset_type == "HardTracksDataset":
            from masa.datasets import HardTracksDataset

            self.class_list = HardTracksDataset.METAINFO["classes"]

        self.cat_ids = self.coco.get_cat_ids(cat_names=self.class_list)
        
        print()
        print("Using the following categories:")
        print("=" * 50)
        for cat_id, cat_name in zip(self.cat_ids, self.class_list):
