...
class LaSotTETAMetric(BaseVideoMetric):
    ...
    default_prefix: Optional[str] = "lasot_teta_metric"
    ...
    def __init__(
        ...
        assert dataset_type in ["LaSotDataset"]
        if dataset_type == "LaSotDataset":
            from masa.datasets import LaSotDataset

            self.class_list = LaSotDataset.METAINFO["classes"]
    ...
    def transform_gt_and_pred(self, img_data_sample):
        ...
        for i in range(len(pred_instances["instances_id"])):
            ...
            data_dict["category_id"] = 1 #TODO: self.cat_ids[pred_instances["labels"][i]]
