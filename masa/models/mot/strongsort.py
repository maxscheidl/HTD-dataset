    ...
    def predict(self,
        ...
        for frame_id in range(video_len):
            ...
            if hasattr(self.detector.backbone, "with_text_model"):
                raise NotImplementedError(
                    'Bytetrack does not support text detection yet.')
            else:
            ...
            for i in range(len(pred_track_instances.instances_id)):
                video_track_instances.append(
                    np.array([
                        ...
                        pred_track_instances.labels[i].cpu()
                    ]))


        
        #print(f"video_track_instances: {np.array(video_track_instances).shape}")
        video_track_instances = np.array(video_track_instances).reshape(-1, 8)
        #print(f"reshaped video_track_instances: {video_track_instances.shape}")

        postproc_input = video_track_instances[:, :7]
        postproc_output = self.postprocess_model.forward(postproc_input)

        labels = video_track_instances[:, 7].reshape(-1, 1)
        video_track_instances = np.hstack((postproc_output, labels))
        #print(f"postproc_output: {postproc_output.shape}")
        #print(f"video_track_instances: {video_track_instances.shape}")
        ...
        for frame_id in range(video_len):
            track_data_sample[frame_id].pred_track_instances = \
                    InstanceData(
                        #bboxes=video_track_instances[video_track_instances[:, 0] == frame_id + 1, :],
                        instances_id=video_track_instances[video_track_instances[:, 0] == frame_id + 1, 1],
                        bboxes=video_track_instances[video_track_instances[:, 0] == frame_id + 1, 2:6],
                        scores=video_track_instances[video_track_instances[:, 0] == frame_id + 1, 6],
                        labels=video_track_instances[video_track_instances[:, 0] == frame_id + 1, 7].astype(int),
                    )

        #print(f"track_data_sample:")
        #print(f"instance_id: {track_data_sample[0].pred_track_instances.instances_id}")
        #print(f"bboxes: {track_data_sample[0].pred_track_instances.bboxes}")
        #print(f"scores: {track_data_sample[0].pred_track_instances.scores}")
        #print(f"labels: {track_data_sample[0].pred_track_instances.labels}")
