...
class ByteTracker(BaseTracker):
    ...
    def track(self, data_sample: DetDataSample, **kwargs) -> InstanceData:
        ...
        # check if width and height are bigger than 1 (bbox has format xyxy)
        if (bboxes[:, 2] - bboxes[:, 0] < 1).any() or (bboxes[:, 3] - bboxes[:, 1] < 1).any():
            print(f"WARNING: Detected bboxes have width or height less than 1.")

        # check if width and height are not 0 (bbox has format xyxy)
        if (bboxes[:, 2] - bboxes[:, 0] == 0).any() or (bboxes[:, 3] - bboxes[:, 1] == 0).any():
            print(f"WARNING: Detected bboxes have width or height equal to 0.")

            # filter out bboxes with 0 height or width
            # Important because this can cause problems with the Kalman filter and produces inf values
            valid_bboxes = (bboxes[:, 2] - bboxes[:, 0] > 0) & (bboxes[:, 3] - bboxes[:, 1] > 0)
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            #print(f"number of first det: {first_det_inds.sum()} with score > {self.obj_score_thrs['high']}")
            ...
            #print(f"length of det_bboxes: {len(bboxes)}")
            #print(f"length of first_det_bboxes: {len(first_det_bboxes)}")
            ...
            #print(f"number of second det: {second_det_inds.sum()} with score > {self.obj_score_thrs['low']}")

            #print(f"length of second_det_bboxes: {len(second_det_bboxes)}")
            ...
            #print(f"number of first match: {valid.sum()}")
            ...
            #print(f"number of tentative match: {valid.sum()}")
            ...
            #print(f"number of second match: {valid.sum()}")
        ...
        try:
            ...
            print(f"bboxes {frame_id}: {bboxes}")
            print(f"transformed bboxes: {bbox_xyxy_to_cxcyah(bboxes)}")
            print(f"labels: {frame_id}: {labels}")
            print(f"scores: {frame_id}: {scores}")
            print(f"ids: {ids}")

            print(f"number of first det: {first_det_inds.sum()} with score > {self.obj_score_thrs['high']}") if 'first_det_inds' in locals() else None
            print(f"length of first_det_bboxes: {len(first_det_bboxes)}") if 'first_det_bboxes' in locals() else None
            print(f"number of second det: {second_det_inds.sum()} with score > {self.obj_score_thrs['low']}") if 'second_det_inds' in locals() else None
            print(f"length of second_det_bboxes: {len(second_det_bboxes)}") if 'second_det_bboxes' in locals() else None
            print(f"number of new tracks: {new_track_inds.sum()}") if 'new_track_inds' in locals() else None

            print(f"self.confirmed_ids: {self.confirmed_ids}")
            print(f"self.unconfirmed_ids: {self.unconfirmed_ids}")
            print(f"self.tracks: {self.tracks}")
            print(f"self.num_tracks: {self.num_tracks}")
