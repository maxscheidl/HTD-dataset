...
class MasaTaoTracker(BaseTracker):
    ...
    def track(
        ...
        **kwargs
        ...
        # check if all bboxes have values which are not infinity
        if torch.isinf(bboxes).any():
            print(f"WARNING: Detected bboxes contain inf values in frame {frame_id}.")
        
        # check if all bboxes are positive
        if (bboxes < 0).any():
            print(f"WARNING: Detected bboxes contain negative values in frame {frame_id}.")

        # check if width and height are not 0 (bbox has format xyxy)
        if (bboxes[:, 2] - bboxes[:, 0] == 0).any() or (bboxes[:, 3] - bboxes[:, 1] == 0).any():
            print(f"WARNING: Detected bboxes have width or height equal to 0 in frame {frame_id}.")
