...
class MasaTaoTrackerPlus(BaseTracker):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.short_term_momentum = short_term_momentum
        self.long_term_momentum = long_term_momentum
        self.matching_beta = matching_beta

        print(f"Using {self.__class__.__name__} as tracker")
    ...
    def update(
        ...
        for id, bbox, embed, label, score in zip(
            ...
            if id in self.tracks.keys():
                ...
                #self.tracks[id]["embed"] = (1 - self.memo_momentum) * self.tracks[id]["embed"] + self.memo_momentum * embed
                self.tracks[id]["embed_short"] = (1 - self.short_term_momentum) * self.tracks[id]["embed_short"] + self.short_term_momentum * embed
                self.tracks[id]["embed_long"] = (1 - self.long_term_momentum) * self.tracks[id]["embed_long"] + self.long_term_momentum * embed
    ...
    def memo(self) -> Tuple[Tensor, ...]:
        ...
        for k, v in self.tracks.items():
            ...
            memo_short_embeds.append(v["embed_short"][None, :])
            memo_long_embeds.append(v["embed_long"][None, :])
        ...
        memo_short_embeds = torch.cat(memo_short_embeds, dim=0)
        memo_long_embeds = torch.cat(memo_long_embeds, dim=0)
    ...
    def track(
        ...
        **kwargs
        ...
        if bboxes.size(0) > 0 and not self.empty:
            ...
            feats_short = torch.mm(embeds, memo_short_embeds.t())
            d2t_scores_short = feats_short.softmax(dim=1)
            t2d_scores_short = feats_short.softmax(dim=0)
            match_scores_bisoftmax_short = (d2t_scores_short + t2d_scores_short) / 2

            match_scores_cosine_short = torch.mm(
                F.normalize(embeds, p=2, dim=1),
                F.normalize(memo_short_embeds, p=2, dim=1).t(),
            )

            match_scores_short = (match_scores_bisoftmax_short + match_scores_cosine_short) / 2

            # --- Long-term similarity ---
            feats_long = torch.mm(embeds, memo_long_embeds.t())
            d2t_scores_long = feats_long.softmax(dim=1)
            t2d_scores_long = feats_long.softmax(dim=0)
            match_scores_bisoftmax_long = (d2t_scores_long + t2d_scores_long) / 2

            match_scores_cosine_long = torch.mm(
                F.normalize(embeds, p=2, dim=1),
                F.normalize(memo_long_embeds, p=2, dim=1).t(),
            )

            match_scores_long = (match_scores_bisoftmax_long + match_scores_cosine_long) / 2

            # --- Combine both similarity scores ---
            match_scores = self.matching_beta * match_scores_short + (1 - self.matching_beta) * match_scores_long
