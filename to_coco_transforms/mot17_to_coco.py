...
def process_video(data_dir, image_root_path, video_folder, shared_data):
    ...
    try:
        ...
        meta_info_file = os.path.join(video_path, "seqinfo.ini")
        image_folder_path = os.path.join(video_path, "img1")
        ...
        print(gt[:2])  # Print first 5 lines of the ground truth file
        ...
        img_width_scraped = re.search(r"imWidth=(\d+)", file_content).group(1)
        img_height_scraped = re.search(r"imHeight=(\d+)", file_content).group(1)
        assert img_width == int(img_width_scraped), f"Width mismatch: {img_width} != {img_width_scraped}"
        assert img_height == int(img_height_scraped), f"Height mismatch: {img_height} != {img_height_scraped}"
        ...
        for class_id in unique_classes:
            if class_id not in [cat['id'] for cat in coco_data["categories"]]:
                class_name = "human"  # Dancetrack has only humans
        ...
        for idx, image_name in enumerate(sorted(image_files)):
            ...
            images.append({
                "id": img_id,
                "video": video_folder,
                "file_name": os.path.join(image_root_path, video_folder, "img1", image_name),
                "width": img_width,
                "height": img_height,
                "frame_index": idx,
                "license": 0,
...
if __name__ == "__main__":
    ...
    data_dir = "../data/mot17_20/mot17/train"
    annotations_dir = "../data/metadata"
    ...
    # Filter out folder which dont end with "DPM"
    video_folders = list(filter(lambda x: x.endswith("DPM"), video_folders))
    ...
    # Save to JSON
    with open(f"{annotations_dir}/mot17_coco_train.json", "w") as json_file:
